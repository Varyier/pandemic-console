...
function parse_req(req) {
  ...
  var inner_req = req.trim();
  if(inner_req == '') {
    return;
  } else if(inner_req.indexOf('/') != 0) {
    alert('Wrong request. Type \'/help\' for more info.');
  } else {
    var pos_cmd_delim = inner_req.indexOf(' ');
    if(pos_cmd_delim == -1) {
      // no args
      req_st.command = inner_req.slice(1, inner_req.length);
    } else {
      // with args, probably
      req_st.command = inner_req.slice(1, pos_cmd_delim);
    }
    ...
    if(pos_cmd_delim != -1) {
      var req_cut = inner_req.slice(pos_cmd_delim+1, inner_req.length);
      if(req_cut.length > 0) {        
        // at least 1 arg
...
function mymain() {
  ...
  var i_val_trimmed = i_val.trim();
  var st = false;
  if(i_val_trimmed.indexOf('/') == 0) {
    // structured request
    parse_req(i_val_trimmed);
    st = true;
  } else if(i_val_trimmed.length > 0) {
    // plain request - parse later
    req_flat = i_val_trimmed;
  }
  
  if(i_val_trimmed.length > 0) {
    if(req_st.command.length > 0 || req_flat != '') {
      ...
      if(State.misc_multistep_request) {
        ...
        if(st) {
          // save the command for convenience
          State.misc_prev_commands.push(i_val);
          State.misc_prev_command_index = -1;
        }
...
//  State.misc_prev_command_index++;
//  if(State.misc_prev_command_index >= State.misc_prev_commands.length) {
//    State.misc_prev_command_index = State.misc_prev_commands.length-1;
//  }
  
//  if(State.misc_prev_command_index >= 0 && State.misc_prev_command_index < State.misc_prev_commands.length) {
//    var index = State.misc_prev_commands.length-1 - State.misc_prev_command_index;
//    document.getElementById('mainInput').value = State.misc_prev_commands[index];
//  }
...
//  if(State.misc_prev_command_index >= 0) {
//    State.misc_prev_command_index--;
//    if(State.misc_prev_command_index < 0) {
//      State.misc_prev_command_index = 0;
//    }
//  }
...
//app.get('/init_state', function(req, res) {
//    // create empty file
...
app.listen(8080)                           // Запускаем сервер на 5000 порту, если не указана переменная окружения "port" 
